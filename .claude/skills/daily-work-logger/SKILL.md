...
## 경로
- Vault: $OBSIDIAN_VAULT/
- 분석 대상 디렉토리: 001_Inbox/, 002_Notes/, work-log/

## 실행 단계
1. Bash로 해당 날짜에 수정된 .md 파일 찾기 (macOS 호환):
   find $OBSIDIAN_VAULT -name "*.md" -type f -exec stat -f "%Sm %N" -t "%Y-%m-%d" {} \; 2>/dev/null | grep "{TARGET_DATE}" | awk '{print $2}' | grep -v $DAILY_NOTE_DIR
...
## 경로
- 미팅 노트 패턴: $OBSIDIAN_VAULT/$DAILY_NOTE_DIR/{TARGET_DATE}-*.md
