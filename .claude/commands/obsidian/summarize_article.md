---
argument-hint: "[url]"
description: "기술 문서 URL → 백그라운드로 번역/정리 → env.config 의 ARTICLE_DIR 을 참고해서  obsidian 문서 생성"
color: yellow
---
...
**이 스킬이 직접 호출된 경우 (사용자가 메인 세션에서 `/obsidian:summarize-article URL` 실행):** → **백그라운드 모드**로 실행
...
### Step 1: 콘텐츠 추출 (WebFetch)

`WebFetch` 도구로 URL에 접근하여 콘텐츠를 추출합니다.
...
WebFetch 결과에 이미지 URL이 포함된 경우, ATTACHMENTS 폴더에 저장하고 Obsidian 문서에 포함시킵니다.
