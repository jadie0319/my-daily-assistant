...
description: "Youtube URL → 백그라운드로 번역/정리 → Obsidian Zettelkasten/youtube/ 저장"
---
...
프로젝트 루트의 `env.config` 파일을 읽어 `OBSIDIAN_VAULT` 경로를 추출한다.

```shell
OBSIDIAN_VAULT=$(grep '^OBSIDIAN_VAULT=' /Users/choejaeyong/my-daily-assistant/env.config | cut -d'=' -f2-)
```

이후 모든 단계에서 `OBSIDIAN_VAULT` 변수를 사용한다.
...
if [[ "$CLEANED_ARGUMENTS" == *"youtube.com/watch?v="* ]] || [[ "$CLEANED_ARGUMENTS" == *"youtu.be/"* ]]; then
    ...
    # Python 스크립트로 JSON 형식 데이터 추출
    SCRIPT_PATH="/Users/choejaeyong/my-daily-assistant/.claude/commands/obsidian/get_youtube_transcript.py"
...
3. **번역 및 요약**
    ...
    - 저장 경로: `{OBSIDIAN_VAULT}/Zettelkasten/youtube/YYYY-MM-DD {영상제목}.md`
    - 디렉터리가 없으면 생성 (`mkdir -p`)
4. **태그 부여**
...
```json
{
  ...
  "output_file": "Zettelkasten/youtube/YYYY-MM-DD 문서제목.md",
  "error": null
}
```
